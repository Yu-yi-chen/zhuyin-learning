...
### R1：題目生成
- 每題從題庫隨機抽一個作為正確答案（符號主題 37 個；結合韻主題 21 個）
- **防重複**：抽題時排除最近 8 題出過的（兩主題皆適用；題庫被排完時重置）
- 提供 4 個選項（含正確答案）
...
### R10：結合韻主題（克漏字格式）
- 題目：從 21 個結合韻隨機抽一個，播放其代表詞音檔（`/audio/compounds/{結合韻}.m4a`，如 ㄩㄥ → 勇士）
- **題面為克漏字**：顯示該詞完整的直式注音，**挖空「等於目標結合韻的那個音節」**（不限位置，
  如 勇士 → 「⬚ ㄕˋ」挖第一格；讚唷 → 「ㄗㄢˋ ⬚」挖第二格），其餘音節注音完整。
  設計理由：整詞多個音節都可能含結合韻（如月亮的 亮=ㄌㄧㄤˋ），不挖空會答案模糊
- 選項：4 個結合韻含聲調（如 ㄩㄥˇ），直式符號呈現 + romanization（小字）
- **干擾選項的聲調與答案一致**（都用答案的聲調）——測的是韻的辨識，聲調不同會變成提示
- **Easy**：干擾從全部 21 個隨機選；**Challenge**：干擾優先**同介音系**（ㄩㄥ → ㄩㄢ ㄩㄣ ㄩㄝ）、
  隱藏 romanization（同符號主題規則）
- **出題提示圖**：Easy 模式在克漏字上方顯示該詞插圖（110px）當提示；Challenge 不顯示
- 答對：空格填入正確結合韻，覆蓋層顯示米白卡片（金棕描邊）內含
  **插圖（110px，兩種難度皆顯示）+ 中文詞 + 完整注音 + 譯文**（取代筆順動畫）
- 積分、confetti、錯選標紅規則與符號主題相同
- 資料前提：代表詞的注音中**必須有一個音節恰等於該結合韻**（無聲母），挖空位置由程式尋找
