...
- 同時播放該符號的注音發音（`playZhuyin(symbol)`，使用 Web Audio API）
...
### R8：顯示代表詞
- 練習頁側邊同步顯示當前符號的代表詞（`WordPanel` 元件）
- 資料來源：`src/data/words.js`，每個符號一筆
- 顯示內容：插圖區（Phase 1 佔位框）、中文詞、直式注音標記（含正確調號位置）、EN/JP 譯文
- 切換符號時 WordPanel 內容同步更新
- 桌機：顯示在筆畫練習區右側；手機：顯示在下方
