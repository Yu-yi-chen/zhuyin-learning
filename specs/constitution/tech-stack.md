...
src/
  ...
  components/   # 共用 UI 元件（無路由職責）
                #   WordPanel.jsx    — 筆順練習頁側邊例詞面板
                #   WordPopover.jsx  — 結合韻/三拼音點擊後的浮動卡片
  contexts/     # React Context（Auth, Lang, Game, Transition）
  data/         # 靜態純資料
                #   zhuyin.js        — 37 個注音符號
                #   i18n.js          — 多語字串
                #   words.js         — 37 個注音代表詞（WordPanel 用）
                #   compounds.js     — 21 個結合韻資料
                #   syllables.js     — 三拼音有效組合表
                #   syllable-words.js — ~120 個三拼音代表詞
...
/compound            → CompoundPage（結合韻練習）
/syllables           → SyllablesPage（三拼音練習）
